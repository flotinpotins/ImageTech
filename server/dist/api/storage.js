...
// 生成唯一的文件名
function generateFileName(extension, prefix = 'img') {
...
function sanitizeMetadata(input) {
    ...
    const sanitizeKey = (key) => key.toLowerCase().replace(/[^a-z0-9-_]/g, '-').slice(0, 128);
...
// 上传图片到对象存储
export async function uploadImageToStorage(dataURL, options) {
...
// 批量上传图片
export async function uploadImagesToStorage(dataURLs, options) {
    const results = await Promise.allSettled(dataURLs.map(dataURL => uploadImageToStorage(dataURL, options)));
